      ...
      <div className='container'>
        {document.map(element => (
          <div className='process' key={element._id}>
            ...
            <p>{element.autor} (Autor) x {element.reu} (Réu)</p>
            <p>{element.tribunal} - {formatDate(element.data)} </p>
            <p>Movimentações:</p>
            <ul>
              {element.movimentacoes.map(movimentacao => (
                <div key={movimentacao._id}>
                  <li className='moviment'>{movimentacao.descricao}: {formatDate(movimentacao.data)}</li>
                </div>
              ))}
            </ul>
          </div>
